...
if (isset($_POST['encrypt_file']) && isset($_FILES['file_to_encrypt'])) {
    $uploadDir = 'uploads/';
    $uploadedFile = $uploadDir . basename($_FILES['file_to_encrypt']['name']);
    $encryptedFile = $uploadDir . 'encrypted_' . basename($_FILES['file_to_encrypt']['name']);

    if (move_uploaded_file($_FILES['file_to_encrypt']['tmp_name'], $uploadedFile)) {
        encrypt_file($uploadedFile, $encryptedFile);
        unlink($uploadedFile); // Hapus file asli setelah enkripsi
        $message = "File berhasil dienkripsi. <a href='$encryptedFile' download>Download File Terenkripsi</a>";
    } else {
        ...
        <div class="row justify-content-center">
            <div class="col-md-8 ">
                <div class="card shadow">
                    <div class="card-body">
                        <h5 class="mb-4 text-center fw-bold text-dark">Enkripsi File</h5>
                        
                        <!-- Form Enkripsi File -->
                        <form method="POST" enctype="multipart/form-data" class="mb-3">
                            <div class="mb-3">
                                <input type="file" id="file_to_encrypt" name="file_to_encrypt" class="form-control" required>
                            </div>
                            <?php if (!empty($message)): ?>
                            <div class="alert alert-info"><?php echo $message; ?></div>
                            <?php endif; ?>
                            <div class="text-center">
                                <button type="submit" name="encrypt_file" class="btn btn-warning text-white mb-3 mt-3">Enkripsi</button>
                            </div>
                        </form>
                    </div>
                </div>
